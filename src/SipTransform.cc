...
SipForwardTransform SipForwardTransform::convert(
    ...
    auto forwardSipPoly = compose(
        geom::AffineTransform(cdMatrix.invert()),
        compose(
            poly,
            geom::AffineTransform(geom::Extent2D(pixelOrigin))
        )
    );
...
SipForwardTransform SipForwardTransform::convert(
    ...
    auto forwardSipPoly = compose(
        geom::AffineTransform(cdMatrix.invert())*scaled.getOutputScalingInverse(),
        compose(
            scaled.getPoly(),
...
geom::AffineTransform SipForwardTransform::linearize(geom::Point2D const & in) const {
    ...
    return geom::AffineTransform(_cdMatrix)
        * (geom::AffineTransform() + _poly.linearize(tail(in)))
        * tail;
}
...
SipReverseTransform SipReverseTransform::convert(
    ...
    auto reverseSipPoly = compose(
        ...
        *scaled.getOutputScalingInverse(),
        compose(
            scaled.getPoly(),
            scaled.getInputScaling()*geom::AffineTransform(cdMatrix)
        )
    );
    // Account for the terms outside the sum in the definition (see comment
    // earlier in the file for more explanation).
...
SipReverseTransform SipReverseTransform::convert(ScaledPolynomialTransform const & scaled) {
    return convert(
        scaled,
        geom::Point2D(scaled.getOutputScalingInverse().getTranslation()),
...
geom::AffineTransform SipReverseTransform::linearize(geom::Point2D const & in) const {
    return geom::AffineTransform(geom::Extent2D(getPixelOrigin()))
        * (geom::AffineTransform() + _poly.linearize(_cdInverse(in)))
        * _cdInverse;
}
...
std::shared_ptr<afw::geom::SkyWcs> rotateWcsPixelsBy90(
    ...
    return transformWcsPixels(
        wcs,
        geom::AffineTransform(rot, offset)
    );
}
