...
def main():
    ...
    for src in refs:
        ...
        xy = wcs1tan.skyToPixel(src.getCoord())
        rx2.append(xy[0])
        ry2.append(xy[1])
    ...
    II,d = match(np.vstack((x,y)).T, np.vstack((rx0,ry0)).T, 10.)
    I = II[:,0]
    J = II[:,1]

    plt.clf()
    plothist(x[I]-rx0[J], y[I]-ry0[J], 200)
    plt.title('Source positions - Reference positions (initial WCS)')
    ps.savefig()

    II,d = match(np.vstack((x,y)).T, np.vstack((rx1,ry1)).T, 10.)
    I = II[:,0]
    J = II[:,1]
    
    plt.clf()
    plothist(x[I]-rx1[J], y[I]-ry1[J], 200)
    plt.title('Source positions - Reference positions (SIP WCS)')
    ps.savefig()

    II,d = match(np.vstack((x,y)).T, np.vstack((rx2,ry2)).T, 10.)
    I = II[:,0]
    J = II[:,1]
    
    plt.clf()
    plothist(x[I]-rx2[J], y[I]-ry2[J], 200)
    ...
    for m in matches:
        ...
        xy = wcs1tan.skyToPixel(ref.getCoord())
        mrx.append(xy[0])
        mry.append(xy[1])
        msx.append(src.getX())
        msy.append(src.getY())
